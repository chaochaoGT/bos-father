...
<script type="text/javascript">
        ...
        $(function() {
            ...
            // 基本功能菜单加载
            $.post("${pageContext.request.contextPath}/menu/menuAction_findMenuByUid",function(data){
            ...
            window.setTimeout(function(){
                $.messager.show({
                    title:"消息提示",
                    msg:"'欢迎登录,&nbsp;'<shiro:principal property='email' />"+"！"+"<a href='javascript:void'  onclick='top.showAbout();'>联系管理员</a>'",
                    timeout:5000
                });
            },3000);
            /*************/
            ...
            $("#btnEp").click(function(){
                var  regix=/\s+/;
                if(regix.test($("#txtNewPass").val()) || $("#txtNewPass").val()==""){
                    $.messager.alert("警告!","请输入有效的密码输入不合法,不能添加空格!", "warning");
                    return;
                }else if(($("#txtRePass").val())=="" || (regix.test($("#txtRePass").val()))){
                    $.messager.alert("警告!","请输入有效的密码输入不合法,不能添加空格", "warning");
                    return;
	...
	function logoutFun() {
		...
		.confirm('系统提示','您确定要退出本次登录吗?',function(isConfirm) {
			if (isConfirm) {
				location.href = '${pageContext.request.contextPath }/user/userAction_outLogin';
			}
		});
	}
	// 修改密码
...
<body class="easyui-layout">
	<div data-options="region:'north',border:false"
		...
		<div id="sessionInfoDiv"
			...
			[<strong id="st_em"><shiro:principal property="email" /></strong>]，欢迎你！您使用[<strong>192.168.1.100</strong>]IP登录！
		</div>
